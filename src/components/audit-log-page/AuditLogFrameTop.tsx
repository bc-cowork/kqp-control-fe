...
type Props = {
  selectedFile: string;
  auditFrame: any; // TODO: define type
  onMaxFrameRefresh: () => void;
};

export function AuditLogFrameTop({ selectedFile, auditFrame, onMaxFrameRefresh }: Props) {
  const theme = useTheme();

  const onClickMaxFrameRefresh = () => {
    onMaxFrameRefresh();
  };
    ...
    <Box>
      <Grid container sx={{ mb: 2 }}>
        ...
        >
          ...
          >{`ms `}</Typography>
          <Typography variant="subtitle1" display="inline">{` ${auditFrame?.time_us}`}</Typography>
